...
// // };
const User = require("../../models/user");
const { Client, Interaction, SlashCommandBuilder, ApplicationCommandOptionType } = require("discord.js");
...
module.exports = {
  ...
  run: async ({client, interaction}) => {
    ...
    try {
      ...
      if (!fetchedBalance) {
        if (targetMember.user.bot) {
          await interaction.editReply("This user doesn't have any coins because they are a bot.");
        } else {
          await interaction.editReply(`${targetMember.displayName} doesn't have any coins yet. Tell them to claim their daily!`);
        }
      } else {
        await interaction.editReply(`**${targetMember.user.tag}** has ${fetchedBalance.balance} coins.`);
      }
      ...
      await interaction.editReply("An error occurred while fetching the user's balance.");
    }
  }
};
